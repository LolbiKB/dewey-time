...
- [x] **[T2-3]** Overnight-shift flag arithmetic — **CONFIRMED CORRECTNESS BUG (3 engine bugs), verified 2026-07-04, sandbox 7 tests (3 pass / 4 fail)**. Evidence: `dewey_time/tests/test_overnight_shifts.py`. **Must-fix if any employee works a midnight-crossing shift.** Root causes:
  - **Bug A — `_get_checkins_for_day` window (closeout.py:677):** fetches D 00:00–23:59 only; D+1 OUT punch is excluded. `checkins_count=1` fails the `>= 2` guard (`closeout.py:510`), so `LATE_START` is silently skipped in production even when the employee arrived demonstrably late. Test `test_late_in_production_mode_single_punch_emits_late_start` FAILS: `flag_codes=[]`.
  - **Bug B — `end_dt` wrong day (closeout.py:560):** `end_dt = combine_date_time(attendance_date, end_time)` maps `end_time=06:00` to `D 06:00` instead of `D+1 06:00`. `early_threshold = D 05:50`; employee who left at `D+1 05:00` (1 hr early) is compared as `D+1 05:00 < D 05:50 = FALSE` → `LEFT_EARLY` never fires. Test `test_early_out_emits_left_early` FAILS: `flag_codes=[]`. Fix: detect `end_time < start_time` in `shift_times.combine_date_time` and add 1 day.
  - **Bug C — `derive_missing_expected_intervals` bails (absence_intervals.py:66):** guard `if end_min <= start_min: return []` short-circuits for any overnight shift (`end_min=360 < start_min=1320`). Also `minutes_from_checkin_time` (attendance_segments.py:47) returns `None` for D+1 punches (`dt.date() != attendance_date`), preventing away-gap detection too. `MISSING_TIME` detection is completely disabled for overnight shifts. Tests `test_derive_missing_expected_returns_intervals_for_overnight_shift` and `test_compute_missing_time_detects_gap_crossing_midnight` both FAIL: `intervals=[]`.
  - **Passing** (3/7): `LATE_START` arithmetic for the start side is correct when both punches are artificially provided; on-time IN/OUT negative cases pass. Coverage gap now closed by `test_overnight_shifts.py`. (found 2026-07-04; PR —)
