...
(`HrAppShell.tsx:22`, currently `/app/attendance-flag`) is **removed** rather than repointed —
the new tab covers that destination, and one nav affordance beats two identical ones.

**A client route alone is not a route.** This corrects an omission in an earlier version of
this spec, which asked only for `main.tsx` and the tab. Frappe serves nothing at a path it has
no rule for, so `/hr-flags` needs all four of the things every other SPA path in this app
already has:

- `dewey_time/www/hr-flags.py` — byte-identical to `www/hr-attendance.py` and
  `www/hr-schedule.py`, which are themselves identical to each other.
- `dewey_time/www/hr-flags.html` — generated by the build, so
  `frontend/hr_attendance/scripts/copy-html-entry.mjs` needs a third `targetHtmlPaths` entry.
- Both `website_route_rules` forms in `hooks.py`, matching the existing ordering:
  `/hr-flags/<path:app_path>` and bare `/hr-flags`.
- A **Python** test pinning the rules and the two files.

That last point is not belt-and-braces. `vite.config.ts` runs the dev server in default SPA
mode, so `/hr-flags` resolves perfectly at `localhost:8080` while 404-ing on the real site —
Playwright cannot see this class of break at all. Any future SPA path added to this app needs
the same four pieces and the same Python-side test.
