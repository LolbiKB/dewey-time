...
```tsx
/**
 * An 08:00–17:00 scheduled week with a 10-minute unworked notch at 10:00–10:10.
 * The window is 07:00–18:00 (660 min), so that notch is 1.5% — under the old
 * 2% floor, which is exactly the case that used to be inflated.
 */
function notchedWeek(): Map<string, Day> {
...
function renderNotched(): string {
  return renderToStaticMarkup(
    <TooltipProvider>
      <WeekView
        weekDates={WEEK}
        daysByDate={notchedWeek()}
...
test("a short band is not inflated to 2% of the window", () => {
  // The floor used to be 2% of the axis — ~14 minutes of apparent time at
  // 07:00-18:00. Unreadable axis, harmless; labelled axis, a visible lie.
  const html = renderNotched();
  for (const [, pct] of html.matchAll(/height:\s*([\d.]+)%/g)) {
    assert.notEqual(Number(pct), 2, "an exactly-2% band means the old floor is still applied");
  }
  assert.match(html, /min-height:\s*3px/, "short bands keep a pixel floor so they stay visible");
});
```
