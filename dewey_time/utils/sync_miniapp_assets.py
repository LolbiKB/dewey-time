...
def _dest_is_the_source(src_dir: str, dest_dir: str) -> bool:
    ...
    can no longer repair it, because `_bundle_ok(src_dir)` now fails too. Only
    a redeploy brings it back. The trigger is an operator running the
    documented repair helper for a bundle that looks stale.
...
def sync_miniapp_assets():
    """Publish the Telegram Mini App bundle from app public/ to sites/assets/."""
    ...
    if not _bundle_ok(src_dir):
        # App ships without the Mini App bundle until the first
        # `npm run build:miniapp` — nothing to sync.
        return
...
def force_sync_miniapp_assets():
    """Unconditionally republish the Mini App bundle (bench console helper)."""
    app_path = frappe.get_app_path(APP)
    src_dir = os.path.join(app_path, "public", BUNDLE)
    dest_dir = os.path.join(frappe.local.sites_path, "assets", APP, BUNDLE)
    ...
    if not _bundle_ok(src_dir):
        frappe.log_error(
            title="force_sync_miniapp_assets missing source",
